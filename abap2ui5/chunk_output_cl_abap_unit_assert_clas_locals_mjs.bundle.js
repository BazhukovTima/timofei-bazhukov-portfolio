"use strict";
(self["webpackChunkabap2ui5_minimal"] = self["webpackChunkabap2ui5_minimal"] || []).push([["output_cl_abap_unit_assert_clas_locals_mjs"],{
...
class lcl_dump {
  ...
  static async to_string(INPUT) {
    ...
    let unique386 = lv_type;
    if (abap.compare.eq(unique386, abap.CharacterFactory.get(1, 'u')) || abap.compare.eq(unique386, abap.CharacterFactory.get(1, 'v'))) {
      rv_str.set((await this.dump_structure({iv_val: iv_val})));
    } else if (abap.compare.eq(unique386, abap.CharacterFactory.get(1, 'h'))) {
      rv_str.set(new abap.types.String().set(`[itab]`));
    } else if (abap.compare.eq(unique386, abap.CharacterFactory.get(1, 'r'))) {
  ...
  static async dump_structure(INPUT) {
    ...
    for await (const unique387 of abap.statements.loop(lt_components)) {
      lv_name.set(unique387);
...
//# sourceMappingURL=chunk_output_cl_abap_unit_assert_clas_locals_mjs.bundle.js.map