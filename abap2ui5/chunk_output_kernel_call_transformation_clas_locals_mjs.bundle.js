"use strict";
(self["webpackChunkabap2ui5_minimal"] = self["webpackChunkabap2ui5_minimal"] || []).push([["output_kernel_call_transformation_clas_locals_mjs"],{
...
class lcl_heap {
  ...
  async add_object(INPUT) {
    ...
    lv_name.set(lo_descr.get().relative_name);
    for await (const unique303 of abap.statements.loop(lo_descr.get().interfaces)) {
      ls_interface.set(unique303);
    ...
    if (abap.compare.eq(is_serializable, abap.builtin.abap_true)) {
      ...
      for await (const unique304 of abap.statements.loop(lo_descr.get().attributes,{where: async (I) => {return abap.compare.eq(I.is_class, abap.builtin.abap_false);},topEquals: {"is_class": abap.builtin.abap_false}})) {
        ls_attribute.set(unique304);
...
class lcl_data_to_xml {
  ...
  async run(INPUT) {
    ...
    let unique305 = lo_type.get().kind;
    if (abap.compare.eq(unique305, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
      for await (const unique306 of abap.statements.loop(lt_comps)) {
        ls_compo.set(unique306);
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
    } else if (abap.compare.eq(unique305, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
    ...
    } else if (abap.compare.eq(unique305, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      ...
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`<${abap.templateFormatting(iv_name)}>`)));
      for await (const unique307 of abap.statements.loop(fs_table_)) {
        fs_any_.assign(unique307);
        lv_ref.assign(fs_any_.getPointer());
        rv_xml.set(abap.operators.concat(rv_xml,(await this.run({iv_name: new abap.types.String().set(`item`), iv_ref: lv_ref}))));
      }
      rv_xml.set(abap.operators.concat(rv_xml,new abap.types.String().set(`</${abap.templateFormatting(iv_name)}>`)));
    } else if (abap.compare.eq(unique305, abap.Classes['CL_ABAP_TYPEDESCR'].kind_ref)) {
      let unique308 = lo_type.get().type_kind;
      if (abap.compare.eq(unique308, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_oref)) {
  ...
  static async run(INPUT) {
    ...
    for (const name in INPUT.source) {
      ...
      static async traverse_write_type(INPUT) {
        ...
        let lo_type = new abap.types.ABAPObject({qualifiedName: "CL_ABAP_TYPEDESCR", RTTIName: "\\CLASS=CL_ABAP_TYPEDESCR"});
        lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
        let unique309 = lo_type.get().type_kind;
        if (abap.compare.eq(unique309, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique309, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique309, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique309, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique309, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique309, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique309, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
      ...
      static async traverse_write(INPUT) {
        ...
        let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
        lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
        let unique310 = lo_type.get().kind;
        if (abap.compare.eq(unique310, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: abap.CharacterFactory.get(6, 'object')});
          await abap.statements.cast(lo_struc, lo_type);
          lt_comps.set((await lo_struc.get().get_components()));
          abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
          for await (const unique311 of abap.statements.loop(lt_comps)) {
            ls_compo.set(unique311);
            abap.statements.assign({component: ls_compo.get().name, target: fs_field_, source: fs_any_});
            lv_ref.assign(fs_field_.getPointer());
          ...
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$close_element();
        } else if (abap.compare.eq(unique310, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$write_value({value: iv_ref.dereference()});
        } else if (abap.compare.eq(unique310, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
          await lcl_object_to_sxml.mi_writer.get().if_sxml_writer$open_element({name: abap.CharacterFactory.get(5, 'array')});
          abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
          for await (const unique312 of abap.statements.loop(fs_table_)) {
            fs_any_.assign(unique312);
            lv_ref.assign(fs_any_.getPointer());
        ...
        if (INPUT.source.constructor.name === "Object") {
            ...
            class lcl_object_to_ixml {
              ...
              static async run(INPUT) {
                ...
                for await (const unique313 of abap.statements.loop(lt_stab)) {
                  ls_stab.set(unique313);
              ...
              static async traverse(INPUT) {
                ...
                let fs_field_ = new abap.types.FieldSymbol(new abap.types.Character(4));
                lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: iv_ref.dereference()})));
                let unique314 = lo_type.get().kind;
                if (abap.compare.eq(unique314, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
                  await abap.statements.cast(lo_struc, lo_type);
                  lt_comps.set((await lo_struc.get().get_components()));
                  abap.statements.assign({target: fs_any_, source: iv_ref.dereference()});
                  for await (const unique315 of abap.statements.loop(lt_comps)) {
                    ls_compo.set(unique315);
                ...
                } else if (abap.compare.eq(unique314, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
                  await ii_parent.get().if_ixml_element$set_value({value: new abap.types.String().set(`${abap.templateFormatting(iv_ref.dereference())}`)});
                } else if (abap.compare.eq(unique314, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
                  abap.statements.assign({target: fs_table_, source: iv_ref.dereference()});
                  for await (const unique316 of abap.statements.loop(fs_table_)) {
                    fs_any_.assign(unique316);
...
//# sourceMappingURL=chunk_output_kernel_call_transformation_clas_locals_mjs.bundle.js.map